...
export default class ProbAccessApplicationCustomizer extends BaseApplicationCustomizer<IProbAccessApplicationCustomizerProperties> {
  ...
  public async onInit(): Promise<void> {
    ...
    try {
      ...
      if (isProtectedB) {
        interface IWebInfoWithVisibility extends IWebInfo {
          Visibility: string;
        }
        console.log('Fetching privacy settings...');
        const privacySetting = await sp.web.select("Title", "Visibility").get() as IWebInfoWithVisibility;
        console.log('Privacy Setting:', privacySetting);
        const isPublic = privacySetting.Visibility === "Public";
